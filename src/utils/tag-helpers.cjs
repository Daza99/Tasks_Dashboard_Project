...
function formatTagDisplay(name) {
  const bare = normalizeTagName(name);
  return bare ? `#${bare}` : '';
}

/** Join tags for UI: `#a #b`. */
function formatTagsDisplay(names) {
 ...
 */
function matchTagPrefix(prefix, catalog, { exclude = [] } = {}) {
 ...
 */
function parseCurrentTagToken(value) {
 ...
 */
function getHashTokenAt(text, caret) {
  const s = String(text || '');
