...
function hydrateTaggedItem(itemType, itemId) {
  ...
  if (itemType === 'bill') return require('./bills').getBill(itemId);
  return null;
}
    ...
    const rows = db
      .prepare(
        `SELECT item_type, item_id FROM (
           ...
           WHERE tg.name = ?
           UNION ALL
           SELECT 'bill', b.id, b.created_at
           FROM bills b
           JOIN item_tags it ON it.item_type = 'bill' AND it.item_id = b.id
           JOIN tags tg ON tg.id = it.tag_id
           WHERE tg.name = ?
         ) attached
      ...
      .all(bare, bare, bare, bare, bare, bare, bare, cap, skip);
    const items = [];
