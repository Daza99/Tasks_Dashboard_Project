...
First run creates `data/` beside the exe: `dashboard.db`, Chromium userData (`data/chromium/`), plus `wallpapers/`, `sounds/`, `exports/`, `themes/`, `backups/`. Nothing is written to `%APPDATA%`. Menubar: **V1.06 (Portable)**.
...
Output: `release/desktop/Personal Dashboard Setup 1.0.6.exe` (per-user NSIS, no admin). Data default: `%APPDATA%\personal-dashboard\data`. Chromium cache stays in AppData. Settings → Data can move the data folder (copy + relaunch; old folder is left in place). Menubar: **V1.06 (Desktop)**.
